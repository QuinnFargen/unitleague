...
statsapi.schedule(date='2019-04-24', start_date=None, end_date=None, team="", opponent="", sportId=1, game_id=None)

statsapi.schedule(date='2025-04-24', start_date=None, end_date=None, team="", opponent="", sportId=1, game_id=None)


statsapi.schedule(date='2019-04-24')

statsapi.boxscore_data(565997, timecode=None)
...
def batting(boxscore_data, gamePk, gamedate):
    ...
    #     return {key.split('-')[0] + '-': value for key, value in notes_data.items()}
    def extract_notes(notes_data):
...
gamePk = 565997
boxscore_data = statsapi.boxscore_data(gamePk, timecode=None)
yesterday = (date.today() - timedelta(days=1)).strftime('%Y-%m-%d')


meta = metaing(boxscore_data, gamePk, yesterday)
bats = batting(boxscore_data, gamePk, yesterday)
pitchs = pitching(boxscore_data, gamePk, yesterday)


meta.to_csv('meta.csv', index=False)
bats.to_csv('bats.csv', index=False)
pitchs.to_csv('pitchs.csv', index=False)
