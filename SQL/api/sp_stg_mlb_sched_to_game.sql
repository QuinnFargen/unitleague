CREATE OR REPLACE PROCEDURE api.sp_stg_mlb_sched_to_game()
 LANGUAGE plpgsql
AS $procedure$
begin


	CREATE TEMP TABLE mlb_sched AS
		select 
			a."GAME_ID",
			a."GAME_DT",
			a."GAME_TIME",
...
-- Put into unknown for duplicate game_id to avoid putting them into GAME twice
	INSERT INTO "api"."stg_mlb_sched_unknown" ("GAME_ID", "GAME_DT", "GAME_TIME", "HOME_TEAM", "AWAY_TEAM", "REASON")
		...
		FROM mlb_sched a
		JOIN (SELECT d."GAME_ID" FROM mlb_sched d GROUP BY d."GAME_ID" HAVING COUNT(*) > 1) b on a."GAME_ID" = b."GAME_ID";

	DELETE
--	SELECT COUNT(*)
	FROM mlb_sched b
	WHERE b."GAME_ID" IN (SELECT "GAME_ID" from "api"."stg_mlb_sched_unknown" c WHERE c."REASON" = 'DUPLICATE GAME_ID');


-- Get TEAM_ID, LEAGUE_ID	
	CREATE TEMP TABLE mlb_sched_teams AS
		select 
			a."GAME_ID",
			a."GAME_DT",
			a."GAME_TIME",
			...
			b."LEAGUE_ID"
		from mlb_sched a
		LEFT JOIN "BALL"."TEAM_NAME" b on a."HOME_TEAM" = b."NAME" and b."LEAGUE_ID" = 4
		LEFT JOIN "BALL"."TEAM_NAME" c on a."AWAY_TEAM" = c."NAME" and c."LEAGUE_ID" = 4
		;
