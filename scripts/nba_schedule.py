...
def merge_data(data):
    ...
    # Split linescores into Q1-Q5 (NBA)
    for i in range(1, 6):
        df[f'Q{i}'] = 0
...
def get_season_calendar(year):
    """
    Pull one anchor day for a season and extract the calendar whitelist.
    """
    anchor_date = f"{year}1015"  # October 15 anchor
    ...
    calendar_dates = league["calendar"]  # whitelist
...
def pull_full_season_from_calendar(year):
    ...
    for day in calendar_days:
        ...
        if games_data:
            df = merge_data(games_data)   # your existing transform
            all_games.append(df)
    ...
    games = collapse_games(results)  # your existing collapse logic
...
final_df.to_csv("nba_full_calendar_schedule.csv", index=False)
